# SLAM-Share Server Container Build Script - PowerShell
# Builds the server container from our successful base image

$ErrorActionPreference = "Stop"

$IMAGE_NAME = "slam-share-server"
$IMAGE_TAG = "latest"
$DOCKERFILE = "Dockerfile.server"

Write-Host "=================================================" -ForegroundColor Cyan
Write-Host "SLAM-Share Server Container Build" -ForegroundColor Cyan
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "✅ SUCCESS: Server container built!" -ForegroundColor Green
        Write-Host "=================================================" -ForegroundColor Green
        Write-Host "Image: $IMAGE_NAME`:$IMAGE_TAG" -ForegroundColor White
        Write-Host "Port: 6767" -ForegroundColor White
        Write-Host "Work Directory: /slam-share-server/received-data" -ForegroundColor White
        Write-Host ""
        Write-Host "To run the server:" -ForegroundColor Yellow
        Write-Host "docker run -p 6767:6767 -d --name slam-server $IMAGE_NAME`:$IMAGE_TAG" -ForegroundColor White
        Write-Host ""
        Write-Host "To test the server:" -ForegroundColor Yellow
        Write-Host ".\\docker\\test-server.ps1" -ForegroundColor White
        Write-Host ""
    }
    else {
